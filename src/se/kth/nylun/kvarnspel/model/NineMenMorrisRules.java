package se.kth.nylun.kvarnspel.model;
...
public class NineMenMorrisRules {
	...
	public NineMenMorrisRules(GameState gs){
		gameplan = gs.getBoard();
		bluemarker = gs.getBluemarker();
		redmarker = gs.getRedmarker();
		turn = gs.getTurn();
	}
	...
	public boolean legalMove(int To, int From, int color) {
		if (color == turn) {
			if (turn == RED_MOVES) {
				//If from starting pile or while pile is still up
				if(redmarker > 0){
					if(From != -1)
						return false;
	...
	/**
	 *  Returns true if the selected player has less than three markers left.
	 */
